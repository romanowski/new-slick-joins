...
trait Tables {
  ...
  class OccupantTable(tag: Tag) extends Table[Occupant](tag, "occupant") {
    def roomId = column[PK[RoomTable]]("room", O.AutoInc)
  ...
  class MessageTable(tag: Tag) extends Table[Message](tag, "message") with NicerJoins[Message, MessageTable] {
  ...
  lazy val db = Database.forConfig("chapter06")

  def exec[T](action: DBIO[T]): T = Await.result(db.run(action), 2 seconds)

  def sqlQuery(q: Query[_, _, Seq]) = q.result.statements.mkString("\n")

  def compare[A](a: A, b: A): Unit ={
    println(a)
    println(b)
    assert(a == b)
  }
